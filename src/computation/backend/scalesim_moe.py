...
config_path = os.path.join(file_path, '../gpt2/input/deepspeedmoe-6.7b-config.json')                        
...
with open(os.path.join(file_path, 'moe_attention.csv'), 'w', newline='') as file:
...
with open(os.path.join(file_path, 'moe_mlp.csv'), 'w', newline='') as file:
...
with open(os.path.join(file_path, 'moe_moe.csv'), 'w', newline='') as file:
    for item in gpt_mlp_tasks:
            line = ','.join(map(str, item)) + ','
            file.write(line + '\n')
