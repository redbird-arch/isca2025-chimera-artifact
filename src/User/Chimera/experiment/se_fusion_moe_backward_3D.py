...
'''
                ...
                for dependency_y in dependency_z:
                    for dependency_x in dependency_y:
                        for dependency_event in dependency_x:
                            if dependency_event not in cluster_attention_weight_events:
                                cluster_attention_weight_events.append(dependency_event)
            cluster_attention_weight_dependency_list = [[[cluster_attention_weight_events for _ in range(cluster_shapes[cluster_step][2])] for _ in range(cluster_shapes[cluster_step][1])] for _ in range(cluster_shapes[cluster_step][0])]
    ...
    if cluster_step < clusters_number - 1:
        ...
        cluster_sub0_pointtopoint_event_tag, cluster_sub0_pointtopoint_dependency_list = manytomany_api.cal_time(
            whole_nodes=node_network, current_event_tag=cluster_sub1_pointtopoint_event_tag, current_dependency_list=cluster_attention_weight_dependency_list,
            source_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step],
