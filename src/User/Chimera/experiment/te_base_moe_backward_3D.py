...
from moe_config_3D import mapping_dict, part_dict, cal_dict
...
'''
    ...
    code += '''
        if cluster_idx == 0:
            if layer_step == 0:
                ...
                cluster_ordertoorder_event_tag, cluster_ordertoorder_dependency_list = ordertoorder_api.cal_time(
                    whole_nodes=node_network, current_event_tag=cluster_sub0_reducescatter_event_tag, current_dependency_list=cluster_reducescatter_dependency_list,
                    source_nodes_coordinates_list=cluster_nodes_lists[cluster_step],
                    source_x_number=cluster_shapes[cluster_step][2], source_y_number=cluster_shapes[cluster_step][1], source_z_number=cluster_shapes[cluster_step][0],
                    topology_x_limitation=node_k, topology_y_limitation=node_k, topology_z_limitation=node_k,
                    data_parallelism_degree=cluster_dataparallels[cluster_step],
        ...
        for dependency_z in cluster_sub0_allreduce_dependency_list:
        ...
        for dependency_z in cluster_sub1_allreduce_dependency_list:
            for dependency_y in dependency_z:
                for dependency_x in dependency_y:
                    for dependency_event in dependency_x:
                        if dependency_event not in cluster_allreduce_events:
                            cluster_allreduce_events.append(dependency_event)
        cluster_allreduce_dependency_list = [[[cluster_allreduce_events for _ in range(cluster_shapes[cluster_step][2])] for _ in range(cluster_shapes[cluster_step][1])] for _ in range(cluster_shapes[cluster_step][0])]

        if layer_step == 0:
            cluster_moe_input_event_tag, cluster_moe_input_dependency_list = compute_3d_base(
                ...
                source_x_number=cluster_shapes[cluster_step][2], source_y_number=cluster_shapes[cluster_step][1], source_z_number=cluster_shapes[cluster_step][0],
                topology_x_limitation=node_k, topology_y_limitation=node_k, topology_z_limitation=node_k,
                message_flits=moe_input_backward_bytes/cluster_shapes[cluster_step][0]/cluster_shapes[cluster_step][1]/cluster_shapes[cluster_step][2],
        ...
        cluster_moe_weight_event_tag, cluster_moe_weight_dependency_list = compute_3d_base(
            ...
            source_x_number=cluster_shapes[cluster_step][2], source_y_number=cluster_shapes[cluster_step][1], source_z_number=cluster_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k, topology_z_limitation=node_k,
            message_flits=moe_weight_backward_bytes/cluster_shapes[cluster_step][0]/cluster_shapes[cluster_step][1]/cluster_shapes[cluster_step][2],
            reduction=computation
        )
        ...
        cluster_sub1_attention_input_event_tag, cluster_sub1_attention_input_dependency_list = compute_3d_base(
            whole_nodes=node_network, current_event_tag=cluster_sub0_allreduce_event_tag, current_dependency_list=cluster_weight_allreduce_dependency_list,
            source_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step],
            source_x_number=cluster_sub1_shapes[cluster_step][2], source_y_number=cluster_sub1_shapes[cluster_step][1], source_z_number=cluster_sub1_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k, topology_z_limitation=node_k,
            message_flits=attention_input_backward_bytes/cluster_sub1_shapes[cluster_step][0]/cluster_sub1_shapes[cluster_step][1]/cluster_sub1_shapes[cluster_step][2],
            reduction=computation
        )

        cluster_sub0_attention_input_event_tag, cluster_sub0_attention_input_dependency_list = compute_3d_base(
            ...
            source_x_number=cluster_sub0_shapes[cluster_step][2], source_y_number=cluster_sub0_shapes[cluster_step][1], source_z_number=cluster_sub0_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k, topology_z_limitation=node_k,
            message_flits=attention_input_backward_bytes/cluster_sub0_shapes[cluster_step][0]/cluster_sub0_shapes[cluster_step][1]/cluster_sub0_shapes[cluster_step][2],
            reduction=computation
        )

        cluster_sub1_attention_weight_event_tag, cluster_sub1_attention_weight_dependency_list = compute_3d_base(
            ...
            source_x_number=cluster_sub1_shapes[cluster_step][2], source_y_number=cluster_sub1_shapes[cluster_step][1], source_z_number=cluster_sub1_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k, topology_z_limitation=node_k,
            message_flits=attention_weight_backward_bytes/cluster_sub1_shapes[cluster_step][0]/cluster_sub1_shapes[cluster_step][1]/cluster_sub1_shapes[cluster_step][2],
            reduction=computation
        )

        cluster_sub0_attention_weight_event_tag, cluster_sub0_attention_weight_dependency_list = compute_3d_base(
            ...
            source_x_number=cluster_sub0_shapes[cluster_step][2], source_y_number=cluster_sub0_shapes[cluster_step][1], source_z_number=cluster_sub0_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k, topology_z_limitation=node_k,
            message_flits=attention_weight_backward_bytes/cluster_sub0_shapes[cluster_step][0]/cluster_sub0_shapes[cluster_step][1]/cluster_sub0_shapes[cluster_step][2],
            reduction=computation
        )
        ...
        for dependency_z in cluster_sub1_attention_weight_dependency_list:
            for dependency_y in dependency_z:
                for dependency_x in dependency_y:
                    for dependency_event in dependency_x:
                        if dependency_event not in cluster_weight_events:
                            cluster_weight_events.append(dependency_event)
        for dependency_z in cluster_sub0_attention_weight_dependency_list:
            for dependency_y in dependency_z:
                for dependency_x in dependency_y:
                    for dependency_event in dependency_x:
                        if dependency_event not in cluster_weight_events:
                            cluster_weight_events.append(dependency_event)
        cluster_weight_dependency_list = [[[cluster_weight_events for _ in range(cluster_shapes[cluster_step][2])] for _ in range(cluster_shapes[cluster_step][1])] for _ in range(cluster_shapes[cluster_step][0])]
        ...
        cluster_sub1_mlp_input_event_tag, cluster_sub1_mlp_input_dependency_list = compute_3d_base(
            whole_nodes=node_network, current_event_tag=cluster_sub0_allreduce_event_tag, current_dependency_list=cluster_weight_allreduce_dependency_list,
            source_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step],
            source_x_number=cluster_sub1_shapes[cluster_step][2], source_y_number=cluster_sub1_shapes[cluster_step][1], source_z_number=cluster_sub1_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k, topology_z_limitation=node_k,
            message_flits=mlp_input_backward_bytes/cluster_sub1_shapes[cluster_step][0]/cluster_sub1_shapes[cluster_step][1]/cluster_sub1_shapes[cluster_step][2],
            reduction=computation
        )

        cluster_sub0_mlp_input_event_tag, cluster_sub0_mlp_input_dependency_list = compute_3d_base(
            ...
            source_x_number=cluster_sub0_shapes[cluster_step][2], source_y_number=cluster_sub0_shapes[cluster_step][1], source_z_number=cluster_sub0_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k, topology_z_limitation=node_k,
            message_flits=mlp_input_backward_bytes/cluster_sub0_shapes[cluster_step][0]/cluster_sub0_shapes[cluster_step][1]/cluster_sub0_shapes[cluster_step][2],
            reduction=computation
        )

        cluster_sub1_mlp_weight_event_tag, cluster_sub1_mlp_weight_dependency_list = compute_3d_base(
            ...
            source_x_number=cluster_sub1_shapes[cluster_step][2], source_y_number=cluster_sub1_shapes[cluster_step][1], source_z_number=cluster_sub1_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k, topology_z_limitation=node_k,
            message_flits=mlp_weight_backward_bytes/cluster_sub1_shapes[cluster_step][0]/cluster_sub1_shapes[cluster_step][1]/cluster_sub1_shapes[cluster_step][2],
            reduction=computation
        )

        cluster_sub0_mlp_weight_event_tag, cluster_sub0_mlp_weight_dependency_list = compute_3d_base(
            ...
            source_x_number=cluster_sub0_shapes[cluster_step][2], source_y_number=cluster_sub0_shapes[cluster_step][1], source_z_number=cluster_sub0_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k, topology_z_limitation=node_k,
            message_flits=mlp_weight_backward_bytes/cluster_sub0_shapes[cluster_step][0]/cluster_sub0_shapes[cluster_step][1]/cluster_sub0_shapes[cluster_step][2],
            reduction=computation
        )
        ...
        for dependency_z in cluster_sub1_mlp_weight_dependency_list:
            for dependency_y in dependency_z:
                for dependency_x in dependency_y:
                    for dependency_event in dependency_x:
                        if dependency_event not in cluster_weight_evnets:
                            cluster_weight_evnets.append(dependency_event)
        for dependency_z in cluster_sub0_mlp_weight_dependency_list:
            for dependency_y in dependency_z:
                for dependency_x in dependency_y:
                    for dependency_event in dependency_x:
                        if dependency_event not in cluster_weight_evnets:
                            cluster_weight_evnets.append(dependency_event)
        cluster_weight_dependency_list = [[[cluster_weight_evnets for _ in range(cluster_shapes[cluster_step][2])] for _ in range(cluster_shapes[cluster_step][1])] for _ in range(cluster_shapes[cluster_step][0])]
        ...
        for sublist1, sublist2 in zip(cluster_weight_dependency_list, cluster_reducescatter_dependency_list):
            merged_sublist = []
            for pair1, pair2 in zip(sublist1, sublist2):
                merged_sublist.append(pair1 + pair2)
            cluster_attention_weight_reducescatter_dependency_list.append(merged_sublist)
        record_event_tags.append(cluster_attention_weight_reducescatter_dependency_list)
    ...
    if cluster_step < clusters_number - 1:
        ...
        cluster_sub1_pointtopoint_event_tag, cluster_sub1_pointtopoint_dependency_list = pointtopoint_api.cal_time(
            whole_nodes=node_network, current_event_tag=cluster_sub0_reducescatter_event_tag, current_dependency_list=cluster_attention_weight_reducescatter_dependency_list,
            source_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step],
            target_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step+1],
            source_x_number=cluster_sub1_shapes[cluster_step][2], source_y_number=cluster_sub1_shapes[cluster_step][1], source_z_number=cluster_sub1_shapes[cluster_step][0],
            target_x_number=cluster_sub1_shapes[cluster_step+1][2], target_y_number=cluster_sub1_shapes[cluster_step+1][1], target_z_number=cluster_sub1_shapes[cluster_step+1][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k, topology_z_limitation=node_k,
            message_flits=pass_bytes/cluster_sub1_shapes[cluster_step][0]/cluster_sub1_shapes[cluster_step][1]/cluster_sub1_shapes[cluster_step][2],
            reduction=reduction_cores
        )

        cluster_sub0_pointtopoint_event_tag, cluster_sub0_pointtopoint_dependency_list = pointtopoint_api.cal_time(
            whole_nodes=node_network, current_event_tag=cluster_sub1_pointtopoint_event_tag, current_dependency_list=cluster_attention_weight_reducescatter_dependency_list,
            source_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step],
            target_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step+1],
            source_x_number=cluster_sub0_shapes[cluster_step][2], source_y_number=cluster_sub0_shapes[cluster_step][1], source_z_number=cluster_sub0_shapes[cluster_step][0],
            target_x_number=cluster_sub0_shapes[cluster_step+1][2], target_y_number=cluster_sub0_shapes[cluster_step+1][1], target_z_number=cluster_sub0_shapes[cluster_step+1][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k, topology_z_limitation=node_k,
            message_flits=pass_bytes/cluster_sub0_shapes[cluster_step][0]/cluster_sub0_shapes[cluster_step][1]/cluster_sub0_shapes[cluster_step][2],
            reduction=reduction_cores
        )

        for sub0_dependency_z in cluster_sub0_pointtopoint_dependency_list:
            for sub0_dependency_y in sub0_dependency_z:
                for sub0_dependency_x in sub0_dependency_y:
                    for sub0_dependency_event in sub0_dependency_x:
                        for sub1_dependency_z in cluster_sub1_pointtopoint_dependency_list:
                            for sub1_dependency_y in sub1_dependency_z:
                                for sub1_dependency_x in sub1_dependency_y:
                                    for sub1_dependency_event in sub1_dependency_x:
                                        if sub0_dependency_event not in sub1_dependency_x:
                                            sub1_dependency_x.append(sub0_dependency_event)
                    ...
                    for sub1_dependency_event in sub1_dependency_x:
                        for sub0_dependency_z in cluster_sub0_pointtopoint_dependency_list:
                            for sub0_dependency_y in sub0_dependency_z:
                                for sub0_dependency_x in sub0_dependency_y:
                                    for sub0_dependency_event in sub0_dependency_x:
                                        if sub1_dependency_event not in sub0_dependency_x:
                                            sub0_dependency_x.append(sub1_dependency_event)

        cluster_pointtopoint_event_tag = cluster_sub1_pointtopoint_event_tag
        cluster_pointtopoint_events = []
        for dependency_z in cluster_sub0_pointtopoint_dependency_list:
            for dependency_y in dependency_z:
                for dependency_x in dependency_y:
                    for dependency_event in dependency_x:
                        if dependency_event not in cluster_pointtopoint_events:
                            cluster_pointtopoint_events.append(dependency_event)
        for dependency_z in cluster_sub1_pointtopoint_dependency_list:
            for dependency_y in dependency_z:
                for dependency_x in dependency_y:
                    for dependency_event in dependency_x:
                        if dependency_event not in cluster_pointtopoint_events:
                            cluster_pointtopoint_events.append(dependency_event)
        cluster_pointtopoint_dependency_list = [[[cluster_pointtopoint_events for _ in range(cluster_shapes[cluster_step+1][2])] for _ in range(cluster_shapes[cluster_step+1][1])] for _ in range(cluster_shapes[cluster_step+1][0])]
