...
'''
    ...
    code += '''
        ...
        cluster_sub0_allgather_event_tag, cluster_sub0_allgather_dependency_list = allgather_api.cal_time(
            whole_nodes=node_network, current_event_tag=cluster_sub1_allgather_event_tag, current_dependency_list=cluster_se_dependency_list,
        ...
        if cluster_idx == 0:
            if layer_step == 0:
                stage_se_event_tag, stage_se_dependency_list = se.fusion(
                    whole_nodes=node_network, current_event_tag=cluster_sub0_attention_weight_event_tag, current_dependency_list=initial_cluster_dependency_lists[cluster_step],
                    source_nodes_coordinates_list=cluster_nodes_lists[cluster_step],
                    source_x_number=cluster_shapes[cluster_step][1], source_y_number=cluster_shapes[cluster_step][0],
                    topology_x_limitation=node_k, topology_y_limitation=node_k,
                    data_parallelism_degree=cluster_dataparallels[cluster_step], top_k=model_config['top_k'],
                ...
                stage_se_event_tag, stage_se_dependency_list = se.fusion(
                    whole_nodes=node_network, current_event_tag=cluster_sub0_attention_weight_event_tag, current_dependency_list=cluster_stage_attention_weight_reducescatter_dependency_list,
                    source_nodes_coordinates_list=cluster_nodes_lists[cluster_step],
                    source_x_number=cluster_shapes[cluster_step][1], source_y_number=cluster_shapes[cluster_step][0],
                    topology_x_limitation=node_k, topology_y_limitation=node_k,
                    data_parallelism_degree=cluster_dataparallels[cluster_step], top_k=model_config['top_k'],
        ...
        stage_sub1_allgather_event_tag, stage_sub1_allgather_dependency_list = allgather_api.cal_time(
            whole_nodes=node_network, current_event_tag=stage_se_event_tag, current_dependency_list=stage_se_dependency_list,
            source_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step],
            source_x_number=cluster_sub1_shapes[cluster_step][1], source_y_number=cluster_sub1_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
        ...
        stage_sub0_allgather_event_tag, stage_sub0_allgather_dependency_list = allgather_api.cal_time(
            whole_nodes=node_network, current_event_tag=stage_sub1_allgather_event_tag, current_dependency_list=stage_se_dependency_list,
            source_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step],
            source_x_number=cluster_sub0_shapes[cluster_step][1], source_y_number=cluster_sub0_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            message_flits=pass_bytes,
        ...
        cluster_allgather_events = []
        for dependency_y in stage_sub0_allgather_dependency_list:
            for dependency_x in dependency_y:
                for dependency_event in dependency_x:
                    if dependency_event not in cluster_allgather_events:
                        cluster_allgather_events.append(dependency_event)
        for dependency_y in stage_sub1_allgather_dependency_list:
            for dependency_x in dependency_y:
                for dependency_event in dependency_x:
                    if dependency_event not in cluster_allgather_events:
                        cluster_allgather_events.append(dependency_event)
        stage_allgather_dependency_list = [[cluster_allgather_events for _ in range(cluster_shapes[cluster_step][1])] for _ in range(cluster_shapes[cluster_step][0])]

        stage_weight_allgather_dependency_list = []
        for sublist1, sublist2 in zip(stage_moe_weight_dependency_list, stage_allgather_dependency_list):
            merged_sublist = []
            for pair1, pair2 in zip(sublist1, sublist2):
                merged_sublist.append(pair1 + pair2)
            stage_weight_allgather_dependency_list.append(merged_sublist)

        stage_sub1_attention_input_event_tag, stage_sub1_attention_input_dependency_list = compute_2d_base(
            whole_nodes=node_network, current_event_tag=stage_sub0_allgather_event_tag, current_dependency_list=stage_weight_allgather_dependency_list,
            source_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step],
            source_x_number=cluster_sub1_shapes[cluster_step][1], source_y_number=cluster_sub1_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            message_flits=attention_input_backward_bytes/cluster_sub1_shapes[cluster_step][0]/cluster_sub1_shapes[cluster_step][1],
        ...
        stage_sub0_attention_input_event_tag, stage_sub0_attention_input_dependency_list = compute_2d_base(
            whole_nodes=node_network, current_event_tag=stage_sub1_attention_input_event_tag, current_dependency_list=stage_weight_allgather_dependency_list,
            source_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step],
            source_x_number=cluster_sub0_shapes[cluster_step][1], source_y_number=cluster_sub0_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            message_flits=attention_input_backward_bytes/cluster_sub0_shapes[cluster_step][0]/cluster_sub0_shapes[cluster_step][1],
        ...
        stage_sub0_allgather_event_tag, stage_sub0_allgather_dependency_list = allgather_api.cal_time(
            whole_nodes=node_network, current_event_tag=stage_sub1_allgather_event_tag, current_dependency_list=stage_sub0_reducescatter_dependency_list,
            source_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step],
            source_x_number=cluster_sub0_shapes[cluster_step][1], source_y_number=cluster_sub0_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            message_flits=pass_bytes,
        ...
        stage_weight_allgather_dependency_list = []
        for sublist1, sublist2 in zip(stage_weight_dependency_list, stage_allgather_dependency_list):
            merged_sublist = []
            for pair1, pair2 in zip(sublist1, sublist2):
                merged_sublist.append(pair1 + pair2)
            stage_weight_allgather_dependency_list.append(merged_sublist)
        ...
        stage_sub0_mlp_input_event_tag, stage_sub0_mlp_input_dependency_list = compute_2d_base(
            whole_nodes=node_network, current_event_tag=stage_sub1_mlp_input_event_tag, current_dependency_list=stage_weight_allgather_dependency_list,
            source_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step],
            source_x_number=cluster_sub0_shapes[cluster_step][1], source_y_number=cluster_sub0_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            message_flits=mlp_input_backward_bytes/cluster_sub0_shapes[cluster_step][0]/cluster_sub0_shapes[cluster_step][1],
        ...
        for dependency_y in stage_sub0_allgather_dependency_list:
            for dependency_x in dependency_y:
                for dependency_event in dependency_x:
                    if dependency_event not in cluster_allgather_events:
                        cluster_allgather_events.append(dependency_event)
        stage_allgather_dependency_list = [[cluster_allgather_events for _ in range(cluster_shapes[cluster_step][1])]
        for _ in range(cluster_shapes[cluster_step][0])]
        ...
        if layer_step < 1:
            cluster_sub1_reducescatter_event_tag, cluster_sub1_reducescatter_dependency_list = reducescatter_api.cal_time(
                whole_nodes=node_network, current_event_tag=stage_sub0_attention_weight_event_tag, current_dependency_list=cluster_sub1_attention_input_dependency_list,
                source_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step],
                source_x_number=cluster_sub1_shapes[cluster_step][1], source_y_number=cluster_sub1_shapes[cluster_step][0],
                topology_x_limitation=node_k, topology_y_limitation=node_k,
                message_flits=pass_bytes,
            ...
            stage_sub1_reducescatter_event_tag, stage_sub1_reducescatter_dependency_list = reducescatter_api.cal_time(
                whole_nodes=node_network, current_event_tag=cluster_sub0_reducescatter_event_tag, current_dependency_list=stage_sub1_attention_input_dependency_list,
                source_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step],
                source_x_number=cluster_sub1_shapes[cluster_step][1], source_y_number=cluster_sub1_shapes[cluster_step][0],
                topology_x_limitation=node_k, topology_y_limitation=node_k,
                message_flits=pass_bytes,
            ...
            cluster_stage_attention_weight_dependency_list = []
            for sublist1, sublist2 in zip(cluster_attention_weight_dependency_list, stage_attention_weight_dependency_list):
                merged_sublist = []
                for pair1, pair2 in zip(sublist1, sublist2):
                    merged_sublist.append(pair1 + pair2)
                cluster_stage_attention_weight_dependency_list.append(merged_sublist)
            record_event_tags.append(cluster_stage_attention_weight_dependency_list)
    ...
    if cluster_step < clusters_number - 1:
        ...
        cluster_sub1_pointtopoint_event_tag, cluster_sub1_pointtopoint_dependency_list = manytomany_api.cal_time(
            whole_nodes=node_network, current_event_tag=stage_sub0_attention_weight_event_tag, current_dependency_list=cluster_stage_attention_weight_dependency_list,
            source_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step],
        ...
        cluster_sub0_pointtopoint_event_tag, cluster_sub0_pointtopoint_dependency_list = manytomany_api.cal_time(
            whole_nodes=node_network, current_event_tag=cluster_sub1_pointtopoint_event_tag, current_dependency_list=cluster_stage_attention_weight_dependency_list,
            source_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step],
        ...
        stage_sub1_pointtopoint_event_tag, stage_sub1_pointtopoint_dependency_list = manytomany_api.cal_time(
            whole_nodes=node_network, current_event_tag=cluster_sub0_pointtopoint_event_tag, current_dependency_list=cluster_stage_attention_weight_dependency_list,
            source_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step],
            target_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step+1],
            source_x_number=cluster_sub1_shapes[cluster_step][1], source_y_number=cluster_sub1_shapes[cluster_step][0],
            target_x_number=cluster_sub1_shapes[cluster_step+1][1], target_y_number=cluster_sub1_shapes[cluster_step+1][0],
        ...
        stage_sub0_pointtopoint_event_tag, stage_sub0_pointtopoint_dependency_list = manytomany_api.cal_time(
            whole_nodes=node_network, current_event_tag=stage_sub1_pointtopoint_event_tag, current_dependency_list=cluster_stage_attention_weight_dependency_list,
            source_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step],
            target_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step+1],
            source_x_number=cluster_sub0_shapes[cluster_step][1], source_y_number=cluster_sub0_shapes[cluster_step][0],
            target_x_number=cluster_sub0_shapes[cluster_step+1][1], target_y_number=cluster_sub0_shapes[cluster_step+1][0],
...
'''
    ...
    file_path = os.path.join(target_folder_py, 'se_fusion_' + hardware_config + '_moe.py')
