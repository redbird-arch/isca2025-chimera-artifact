...
'''
    ...
    code += '''
        if cluster_idx == 0:
            if layer_step == 0:
                ...
                cluster_sub1_reducescatter_event_tag, cluster_sub1_reducescatter_dependency_list = reducescatter_api.cal_time(
                    whole_nodes=node_network, current_event_tag=cluster_sub0_reducescatter_event_tag, current_dependency_list=cluster_attention_input_dependency_list,
                    source_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step],
                    source_x_number=cluster_sub1_shapes[cluster_step][2], source_y_number=cluster_sub1_shapes[cluster_step][1], source_z_number=cluster_sub1_shapes[cluster_step][0],
                    topology_x_limitation=node_k, topology_y_limitation=node_k, topology_z_limitation=node_k,
                    message_flits=pass_bytes,
            ...
            if layer_step == 0:
                cluster_sub0_reducescatter_event_tag, cluster_sub0_reducescatter_dependency_list = reducescatter_api.cal_time(
                    whole_nodes=node_network, current_event_tag=cluster_sub0_pointtopoint_event_tag, current_dependency_list=cluster_pointtopoint_dependency_list,
                    source_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step],
                    source_x_number=cluster_sub0_shapes[cluster_step][2], source_y_number=cluster_sub0_shapes[cluster_step][1], source_z_number=cluster_sub0_shapes[cluster_step][0],
                    topology_x_limitation=node_k, topology_y_limitation=node_k, topology_z_limitation=node_k,
                    message_flits=pass_bytes,
                ...
                cluster_sub1_reducescatter_event_tag, cluster_sub1_reducescatter_dependency_list = reducescatter_api.cal_time(
                    whole_nodes=node_network, current_event_tag=cluster_sub0_reducescatter_event_tag, current_dependency_list=cluster_pointtopoint_dependency_list,
                    source_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step],
                    source_x_number=cluster_sub1_shapes[cluster_step][2], source_y_number=cluster_sub1_shapes[cluster_step][1], source_z_number=cluster_sub1_shapes[cluster_step][0],
                    topology_x_limitation=node_k, topology_y_limitation=node_k, topology_z_limitation=node_k,
                    message_flits=pass_bytes,
        ...
        if layer_step == 0:
            ...
            for sublist1, sublist2 in zip(cluster_weight_dependency_list, cluster_se_dependency_list):
                merged_sublist = []
        ...
        for dependency_z in cluster_sub1_attention_input_dependency_list:
            for dependency_y in dependency_z:
                for dependency_x in dependency_y:
                    for dependency_event in dependency_x:
                        if dependency_event not in cluster_attention_input_events:
                            cluster_attention_input_events.append(dependency_event)
        for dependency_z in cluster_sub0_attention_input_dependency_list:
            for dependency_y in dependency_z:
                for dependency_x in dependency_y:
                    for dependency_event in dependency_x:
                        if dependency_event not in cluster_attention_input_events:
                            cluster_attention_input_events.append(dependency_event)
        cluster_attention_input_dependency_list = [[[cluster_attention_input_events for _ in range(cluster_shapes[cluster_step][2])] for _ in range(cluster_shapes[cluster_step][1])] for _ in range(cluster_shapes[cluster_step][0])]
                ...
                for dependency_x in dependency_y:
                    for dependency_event in dependency_x:
                        if dependency_event not in cluster_weight_events:
                            cluster_weight_events.append(dependency_event)
        cluster_weight_dependency_list = [[[cluster_weight_events for _ in range(cluster_shapes[cluster_step][2])] for _ in range(cluster_shapes[cluster_step][1])] for _ in range(cluster_shapes[cluster_step][0])]
        record_event_tags.append(cluster_weight_dependency_list)
    ...
    if cluster_step < clusters_number - 1:
        ...
        cluster_sub1_pointtopoint_event_tag, cluster_sub1_pointtopoint_dependency_list = pointtopoint_api.cal_time(
            whole_nodes=node_network, current_event_tag=cluster_sub0_attention_weight_event_tag, current_dependency_list=cluster_weight_dependency_list,
            source_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step],
        ...
        cluster_sub0_pointtopoint_event_tag, cluster_sub0_pointtopoint_dependency_list = pointtopoint_api.cal_time(
            whole_nodes=node_network, current_event_tag=cluster_sub1_pointtopoint_event_tag, current_dependency_list=cluster_weight_dependency_list,
            source_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step],
