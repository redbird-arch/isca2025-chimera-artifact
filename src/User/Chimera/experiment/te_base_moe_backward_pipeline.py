...
target_folder = os.path.join(file_path, './ISCA25_Backward-Pipeline')
target_folder_cfg = os.path.join(file_path, './ISCA25_Backward-Pipeline/cfg')
target_folder_txt = os.path.join(file_path, './ISCA25_Backward-Pipeline/txt')
target_folder_py = os.path.join(file_path, './ISCA25_Backward-Pipeline/py')
...
hardware_configs = ['a100', 'dojo', 'tpuv3']
...
'''
    ...
    code += '''
        if cluster_idx == 0:
            if layer_step == 0:
                cluster_ordertoorder_event_tag, cluster_ordertoorder_dependency_list = ordertoorder_api.cal_time(
                    whole_nodes=node_network, current_event_tag=initial_event_tag, current_dependency_list=initial_cluster_dependency_lists[cluster_step],
                    ...
                    data_parallelism_degree=cluster_dataparallels[cluster_step],
                ...
                cluster_ordertoorder_event_tag, cluster_ordertoorder_dependency_list = ordertoorder_api.cal_time(
                    whole_nodes=node_network, current_event_tag=stage_sub0_reducescatter_event_tag, current_dependency_list=cluster_stage_attention_weight_reducescatter_dependency_list,
                    source_nodes_coordinates_list=cluster_nodes_lists[cluster_step],
                    source_x_number=cluster_shapes[cluster_step][1], source_y_number=cluster_shapes[cluster_step][0],
                    topology_x_limitation=node_k, topology_y_limitation=node_k,
                    data_parallelism_degree=cluster_dataparallels[cluster_step],
            ...
            if layer_step == 0:
                cluster_ordertoorder_event_tag, cluster_ordertoorder_dependency_list = ordertoorder_api.cal_time(
                    whole_nodes=node_network, current_event_tag=stage_sub0_pointtopoint_event_tag, current_dependency_list=cluster_stage_pointtopoint_dependency_list,
                    source_nodes_coordinates_list=cluster_nodes_lists[cluster_step],
                    source_x_number=cluster_shapes[cluster_step][1], source_y_number=cluster_shapes[cluster_step][0],
                    topology_x_limitation=node_k, topology_y_limitation=node_k,
                    data_parallelism_degree=cluster_dataparallels[cluster_step],
        ...
        cluster_sub1_allreduce_event_tag, cluster_sub1_allreduce_dependency_list = allreduce_api.cal_time(
            whole_nodes=node_network, current_event_tag=cluster_ordertoorder_event_tag, current_dependency_list=cluster_ordertoorder_dependency_list,
        ...
        cluster_sub0_allreduce_event_tag, cluster_sub0_allreduce_dependency_list = allreduce_api.cal_time(
            whole_nodes=node_network, current_event_tag=cluster_sub1_allreduce_event_tag, current_dependency_list=cluster_ordertoorder_dependency_list,
        ...
        cluster_allreduce_events = []
        for dependency_y in cluster_sub0_allreduce_dependency_list:
            for dependency_x in dependency_y:
                for dependency_event in dependency_x:
                    if dependency_event not in cluster_allreduce_events:
                        cluster_allreduce_events.append(dependency_event)
        for dependency_y in cluster_sub1_allreduce_dependency_list:
            for dependency_x in dependency_y:
                for dependency_event in dependency_x:
                    if dependency_event not in cluster_allreduce_events:
                        cluster_allreduce_events.append(dependency_event)
        cluster_allreduce_dependency_list = [[cluster_allreduce_events for _ in range(cluster_shapes[cluster_step][1])] for _ in range(cluster_shapes[cluster_step][0])]

        if layer_step == 0:
            cluster_moe_input_event_tag, cluster_moe_input_dependency_list = compute_2d_base(
                whole_nodes=node_network, current_event_tag=cluster_sub0_allreduce_event_tag, current_dependency_list=cluster_allreduce_dependency_list,
                source_nodes_coordinates_list=cluster_nodes_lists[cluster_step],
                source_x_number=cluster_shapes[cluster_step][1], source_y_number=cluster_shapes[cluster_step][0],
                topology_x_limitation=node_k, topology_y_limitation=node_k,
            ...
            cluster_moe_input_event_tag, cluster_moe_input_dependency_list = compute_2d_base(
                whole_nodes=node_network, current_event_tag=cluster_sub0_allreduce_event_tag, current_dependency_list=cluster_weight_allreduce_dependency_list,
                source_nodes_coordinates_list=cluster_nodes_lists[cluster_step],
                source_x_number=cluster_shapes[cluster_step][1], source_y_number=cluster_shapes[cluster_step][0],
                topology_x_limitation=node_k, topology_y_limitation=node_k,
                message_flits=moe_input_backward_bytes/cluster_shapes[cluster_step][0]/cluster_shapes[cluster_step][1],
        ...
        cluster_ordertoorder_event_tag, cluster_ordertoorder_dependency_list = ordertoorder_api.cal_time(
            whole_nodes=node_network, current_event_tag=cluster_moe_weight_event_tag, current_dependency_list=cluster_moe_weight_dependency_list,
            source_nodes_coordinates_list=cluster_nodes_lists[cluster_step],
            source_x_number=cluster_shapes[cluster_step][1], source_y_number=cluster_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            data_parallelism_degree=cluster_dataparallels[cluster_step],
        ...
        cluster_weight_allreduce_dependency_list = []
        for sublist1, sublist2 in zip(cluster_moe_weight_dependency_list, cluster_allreduce_dependency_list):
            merged_sublist = []
            for pair1, pair2 in zip(sublist1, sublist2):
                merged_sublist.append(pair1 + pair2)
            cluster_weight_allreduce_dependency_list.append(merged_sublist)
        ...
        cluster_sub0_attention_input_event_tag, cluster_sub0_attention_input_dependency_list = compute_2d_base(
            whole_nodes=node_network, current_event_tag=cluster_sub1_attention_input_event_tag, current_dependency_list=cluster_weight_allreduce_dependency_list,
            source_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step],
            source_x_number=cluster_sub0_shapes[cluster_step][1], source_y_number=cluster_sub0_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
        ...
        cluster_sub0_allreduce_event_tag, cluster_sub0_allreduce_dependency_list = allreduce_api.cal_time(
            whole_nodes=node_network, current_event_tag=cluster_sub1_allreduce_event_tag, current_dependency_list=cluster_sub0_reducescatter_dependency_list,
            source_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step],
            source_x_number=cluster_sub0_shapes[cluster_step][1], source_y_number=cluster_sub0_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            message_flits=pass_bytes,
        ...
        cluster_weight_allreduce_dependency_list = []
        for sublist1, sublist2 in zip(cluster_weight_dependency_list, cluster_allreduce_dependency_list):
            merged_sublist = []
            for pair1, pair2 in zip(sublist1, sublist2):
                merged_sublist.append(pair1 + pair2)
            cluster_weight_allreduce_dependency_list.append(merged_sublist)
        ...
        cluster_sub0_mlp_input_event_tag, cluster_sub0_mlp_input_dependency_list = compute_2d_base(
            whole_nodes=node_network, current_event_tag=cluster_sub1_mlp_input_event_tag, current_dependency_list=cluster_weight_allreduce_dependency_list,
            source_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step],
            source_x_number=cluster_sub0_shapes[cluster_step][1], source_y_number=cluster_sub0_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
        ...
        for dependency_y in cluster_sub0_allreduce_dependency_list:
            for dependency_x in dependency_y:
                for dependency_event in dependency_x:
                    if dependency_event not in cluster_allreduce_events:
                        cluster_allreduce_events.append(dependency_event)
        cluster_allreduce_dependency_list = [[cluster_allreduce_events for _ in range(cluster_shapes[cluster_step][1])]
        ...
        cluster_attention_weight_reducescatter_dependency_list = []
        for sublist1, sublist2 in zip(cluster_attention_weight_dependency_list, cluster_reducescatter_dependency_list):
            merged_sublist = []
            for pair1, pair2 in zip(sublist1, sublist2):
                merged_sublist.append(pair1 + pair2)
            cluster_attention_weight_reducescatter_dependency_list.append(merged_sublist)

        if cluster_idx == 0:
            if layer_step == 0:
                stage_ordertoorder_event_tag, stage_ordertoorder_dependency_list = ordertoorder_api.cal_time(
                    whole_nodes=node_network, current_event_tag=cluster_sub0_reducescatter_event_tag, current_dependency_list=initial_cluster_dependency_lists[cluster_step],
                    source_nodes_coordinates_list=cluster_nodes_lists[cluster_step],
                    source_x_number=cluster_shapes[cluster_step][1], source_y_number=cluster_shapes[cluster_step][0],
                    topology_x_limitation=node_k, topology_y_limitation=node_k,
                    data_parallelism_degree=cluster_dataparallels[cluster_step],
                ...
                stage_ordertoorder_event_tag, stage_ordertoorder_dependency_list = ordertoorder_api.cal_time(
                    whole_nodes=node_network, current_event_tag=cluster_sub0_reducescatter_event_tag, current_dependency_list=cluster_stage_attention_weight_reducescatter_dependency_list,
                    source_nodes_coordinates_list=cluster_nodes_lists[cluster_step],
                    source_x_number=cluster_shapes[cluster_step][1], source_y_number=cluster_shapes[cluster_step][0],
                    topology_x_limitation=node_k, topology_y_limitation=node_k,
                    data_parallelism_degree=cluster_dataparallels[cluster_step],
            ...
            if layer_step == 0:
                stage_ordertoorder_event_tag, stage_ordertoorder_dependency_list = ordertoorder_api.cal_time(
                    whole_nodes=node_network, current_event_tag=cluster_sub0_reducescatter_event_tag, current_dependency_list=cluster_stage_pointtopoint_dependency_list,
                    source_nodes_coordinates_list=cluster_nodes_lists[cluster_step],
                    source_x_number=cluster_shapes[cluster_step][1], source_y_number=cluster_shapes[cluster_step][0],
                    topology_x_limitation=node_k, topology_y_limitation=node_k,
                    data_parallelism_degree=cluster_dataparallels[cluster_step],
        ...
        stage_sub1_allreduce_event_tag, stage_sub1_allreduce_dependency_list = allreduce_api.cal_time(
            whole_nodes=node_network, current_event_tag=stage_ordertoorder_event_tag, current_dependency_list=stage_ordertoorder_dependency_list,
            source_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step],
            source_x_number=cluster_sub1_shapes[cluster_step][1], source_y_number=cluster_sub1_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            message_flits=pass_bytes,
        ...
        stage_sub0_allreduce_event_tag, stage_sub0_allreduce_dependency_list = allreduce_api.cal_time(
            whole_nodes=node_network, current_event_tag=stage_sub1_allreduce_event_tag, current_dependency_list=stage_ordertoorder_dependency_list,
            source_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step],
            source_x_number=cluster_sub0_shapes[cluster_step][1], source_y_number=cluster_sub0_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            message_flits=pass_bytes,
        ...
        cluster_allreduce_events = []
        for dependency_y in stage_sub0_allreduce_dependency_list:
            for dependency_x in dependency_y:
                for dependency_event in dependency_x:
                    if dependency_event not in cluster_allreduce_events:
                        cluster_allreduce_events.append(dependency_event)
        for dependency_y in stage_sub1_allreduce_dependency_list:
            for dependency_x in dependency_y:
                for dependency_event in dependency_x:
                    if dependency_event not in cluster_allreduce_events:
                        cluster_allreduce_events.append(dependency_event)
        stage_allreduce_dependency_list = [[cluster_allreduce_events for _ in range(cluster_shapes[cluster_step][1])] for _ in range(cluster_shapes[cluster_step][0])]

        if layer_step == 0:
            stage_moe_input_event_tag, stage_moe_input_dependency_list = compute_2d_base(
                whole_nodes=node_network, current_event_tag=stage_sub0_allreduce_event_tag, current_dependency_list=stage_allreduce_dependency_list,
                source_nodes_coordinates_list=cluster_nodes_lists[cluster_step],
                source_x_number=cluster_shapes[cluster_step][1], source_y_number=cluster_shapes[cluster_step][0],
                topology_x_limitation=node_k, topology_y_limitation=node_k,
                message_flits=moe_input_backward_bytes/cluster_shapes[cluster_step][0]/cluster_shapes[cluster_step][1],
            ...
            stage_moe_input_event_tag, stage_moe_input_dependency_list = compute_2d_base(
                whole_nodes=node_network, current_event_tag=stage_sub0_allreduce_event_tag, current_dependency_list=stage_weight_allreduce_dependency_list,
                source_nodes_coordinates_list=cluster_nodes_lists[cluster_step],
                source_x_number=cluster_shapes[cluster_step][1], source_y_number=cluster_shapes[cluster_step][0],
                topology_x_limitation=node_k, topology_y_limitation=node_k,
                message_flits=moe_input_backward_bytes/cluster_shapes[cluster_step][0]/cluster_shapes[cluster_step][1],
        ...
        stage_moe_weight_event_tag, stage_moe_weight_dependency_list = compute_2d_base(
            whole_nodes=node_network, current_event_tag=stage_moe_input_event_tag, current_dependency_list=stage_moe_input_dependency_list,
            source_nodes_coordinates_list=cluster_nodes_lists[cluster_step],
            source_x_number=cluster_shapes[cluster_step][1], source_y_number=cluster_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            message_flits=moe_weight_backward_bytes/cluster_shapes[cluster_step][0]/cluster_shapes[cluster_step][1],
        ...
        stage_ordertoorder_event_tag, stage_ordertoorder_dependency_list = ordertoorder_api.cal_time(
            whole_nodes=node_network, current_event_tag=stage_moe_weight_event_tag, current_dependency_list=stage_moe_weight_dependency_list,
            source_nodes_coordinates_list=cluster_nodes_lists[cluster_step],
            source_x_number=cluster_shapes[cluster_step][1], source_y_number=cluster_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            data_parallelism_degree=cluster_dataparallels[cluster_step],
        ...
        stage_weight_allreduce_dependency_list = []
        for sublist1, sublist2 in zip(stage_moe_weight_dependency_list, stage_allreduce_dependency_list):
            merged_sublist = []
            for pair1, pair2 in zip(sublist1, sublist2):
                merged_sublist.append(pair1 + pair2)
            stage_weight_allreduce_dependency_list.append(merged_sublist)

        stage_sub1_attention_input_event_tag, stage_sub1_attention_input_dependency_list = compute_2d_base(
            whole_nodes=node_network, current_event_tag=stage_sub0_allreduce_event_tag, current_dependency_list=stage_weight_allreduce_dependency_list,
            source_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step],
            source_x_number=cluster_sub1_shapes[cluster_step][1], source_y_number=cluster_sub1_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
        ...
        stage_sub0_attention_input_event_tag, stage_sub0_attention_input_dependency_list = compute_2d_base(
            whole_nodes=node_network, current_event_tag=stage_sub1_attention_input_event_tag, current_dependency_list=stage_weight_allreduce_dependency_list,
            source_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step],
            source_x_number=cluster_sub0_shapes[cluster_step][1], source_y_number=cluster_sub0_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            message_flits=attention_input_backward_bytes/cluster_sub0_shapes[cluster_step][0]/cluster_sub0_shapes[cluster_step][1],
        ...
        stage_sub1_attention_weight_event_tag, stage_sub1_attention_weight_dependency_list = compute_2d_base(
            whole_nodes=node_network, current_event_tag=stage_sub0_attention_input_event_tag, current_dependency_list=stage_sub1_attention_input_dependency_list,
            source_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step],
            source_x_number=cluster_sub1_shapes[cluster_step][1], source_y_number=cluster_sub1_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            message_flits=attention_weight_backward_bytes/cluster_sub1_shapes[cluster_step][0]/cluster_sub1_shapes[cluster_step][1],
        ...
        stage_sub0_attention_weight_event_tag, stage_sub0_attention_weight_dependency_list = compute_2d_base(
            whole_nodes=node_network, current_event_tag=stage_sub1_attention_weight_event_tag, current_dependency_list=stage_sub0_attention_input_dependency_list,
            source_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step],
            source_x_number=cluster_sub0_shapes[cluster_step][1], source_y_number=cluster_sub0_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            message_flits=attention_weight_backward_bytes/cluster_sub0_shapes[cluster_step][0]/cluster_sub0_shapes[cluster_step][1],
        ...
        stage_sub1_reducescatter_event_tag, stage_sub1_reducescatter_dependency_list = reducescatter_api.cal_time(
            whole_nodes=node_network, current_event_tag=stage_sub0_attention_weight_event_tag, current_dependency_list=stage_sub1_attention_input_dependency_list,
            source_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step],
            source_x_number=cluster_sub1_shapes[cluster_step][1], source_y_number=cluster_sub1_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            message_flits=pass_bytes,
        ...
        stage_sub0_reducescatter_event_tag, stage_sub0_reducescatter_dependency_list = reducescatter_api.cal_time(
            whole_nodes=node_network, current_event_tag=stage_sub1_reducescatter_event_tag, current_dependency_list=stage_sub0_attention_input_dependency_list,
            source_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step],
            source_x_number=cluster_sub0_shapes[cluster_step][1], source_y_number=cluster_sub0_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            message_flits=pass_bytes,
        ...
        stage_sub1_allreduce_event_tag, stage_sub1_allreduce_dependency_list = allreduce_api.cal_time(
            whole_nodes=node_network, current_event_tag=stage_sub0_reducescatter_event_tag, current_dependency_list=stage_sub1_reducescatter_dependency_list,
            source_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step],
            source_x_number=cluster_sub1_shapes[cluster_step][1], source_y_number=cluster_sub1_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            message_flits=pass_bytes,
        ...
        stage_sub0_allreduce_event_tag, stage_sub0_allreduce_dependency_list = allreduce_api.cal_time(
            whole_nodes=node_network, current_event_tag=stage_sub1_allreduce_event_tag, current_dependency_list=stage_sub0_reducescatter_dependency_list,
            source_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step],
            source_x_number=cluster_sub0_shapes[cluster_step][1], source_y_number=cluster_sub0_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            message_flits=pass_bytes,
        ...
        cluster_weight_events = []
        for dependency_y in stage_sub1_attention_weight_dependency_list:
            for dependency_x in dependency_y:
                for dependency_event in dependency_x:
                    if dependency_event not in cluster_weight_events:
                        cluster_weight_events.append(dependency_event)
        for dependency_y in stage_sub0_attention_weight_dependency_list:
            for dependency_x in dependency_y:
                for dependency_event in dependency_x:
                    if dependency_event not in cluster_weight_events:
                        cluster_weight_events.append(dependency_event)
        stage_weight_dependency_list = [[cluster_weight_events for _ in range(cluster_shapes[cluster_step][1])] for _ in range(cluster_shapes[cluster_step][0])]

        stage_weight_allreduce_dependency_list = []
        for sublist1, sublist2 in zip(stage_weight_dependency_list, stage_allreduce_dependency_list):
            merged_sublist = []
            for pair1, pair2 in zip(sublist1, sublist2):
                merged_sublist.append(pair1 + pair2)
            stage_weight_allreduce_dependency_list.append(merged_sublist)

        stage_sub1_mlp_input_event_tag, stage_sub1_mlp_input_dependency_list = compute_2d_base(
            whole_nodes=node_network, current_event_tag=stage_sub0_allreduce_event_tag, current_dependency_list=stage_weight_allreduce_dependency_list,
            source_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step],
            source_x_number=cluster_sub1_shapes[cluster_step][1], source_y_number=cluster_sub1_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
        ...
        stage_sub0_mlp_input_event_tag, stage_sub0_mlp_input_dependency_list = compute_2d_base(
            whole_nodes=node_network, current_event_tag=stage_sub1_mlp_input_event_tag, current_dependency_list=stage_weight_allreduce_dependency_list,
            source_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step],
            source_x_number=cluster_sub0_shapes[cluster_step][1], source_y_number=cluster_sub0_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            message_flits=mlp_input_backward_bytes/cluster_sub0_shapes[cluster_step][0]/cluster_sub0_shapes[cluster_step][1],
        ...
        stage_sub1_mlp_weight_event_tag, stage_sub1_mlp_weight_dependency_list = compute_2d_base(
            whole_nodes=node_network, current_event_tag=stage_sub0_mlp_input_event_tag, current_dependency_list=stage_sub1_mlp_input_dependency_list,
            source_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step],
            source_x_number=cluster_sub1_shapes[cluster_step][1], source_y_number=cluster_sub1_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            message_flits=mlp_weight_backward_bytes/cluster_sub1_shapes[cluster_step][0]/cluster_sub1_shapes[cluster_step][1],
        ...
        stage_sub0_mlp_weight_event_tag, stage_sub0_mlp_weight_dependency_list = compute_2d_base(
            whole_nodes=node_network, current_event_tag=stage_sub1_mlp_weight_event_tag, current_dependency_list=stage_sub0_mlp_input_dependency_list,
            source_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step],
            source_x_number=cluster_sub0_shapes[cluster_step][1], source_y_number=cluster_sub0_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            message_flits=mlp_weight_backward_bytes/cluster_sub0_shapes[cluster_step][0]/cluster_sub0_shapes[cluster_step][1],
        ...
        stage_sub1_reducescatter_event_tag, stage_sub1_reducescatter_dependency_list = reducescatter_api.cal_time(
            whole_nodes=node_network, current_event_tag=stage_sub0_mlp_weight_event_tag, current_dependency_list=stage_sub1_mlp_input_dependency_list,
            source_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step],
            source_x_number=cluster_sub1_shapes[cluster_step][1], source_y_number=cluster_sub1_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            message_flits=pass_bytes,
        ...
        stage_sub0_reducescatter_event_tag, stage_sub0_reducescatter_dependency_list = reducescatter_api.cal_time(
            whole_nodes=node_network, current_event_tag=stage_sub1_reducescatter_event_tag, current_dependency_list=stage_sub0_mlp_input_dependency_list,
            source_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step],
            source_x_number=cluster_sub0_shapes[cluster_step][1], source_y_number=cluster_sub0_shapes[cluster_step][0],
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            message_flits=pass_bytes,
        ...
        for dependency_y in stage_sub0_allreduce_dependency_list:
            for dependency_x in dependency_y:
                for dependency_event in dependency_x:
                    if dependency_event not in cluster_allreduce_events:
                        cluster_allreduce_events.append(dependency_event)
        stage_allreduce_dependency_list = [[cluster_allreduce_events for _ in range(cluster_shapes[cluster_step][1])]
        for _ in range(cluster_shapes[cluster_step][0])]

        cluster_weight_evnets = []
        for dependency_y in stage_sub1_mlp_weight_dependency_list:
            for dependency_x in dependency_y:
                for dependency_event in dependency_x:
                    if dependency_event not in cluster_weight_evnets:
                        cluster_weight_evnets.append(dependency_event)
        for dependency_y in stage_sub0_mlp_weight_dependency_list:
            for dependency_x in dependency_y:
                for dependency_event in dependency_x:
                    if dependency_event not in cluster_weight_evnets:
                        cluster_weight_evnets.append(dependency_event)
        stage_weight_dependency_list = [[cluster_weight_evnets for _ in range(cluster_shapes[cluster_step][1])] for _ in range(cluster_shapes[cluster_step][0])]
        ...
        cluster_reducescatter_events = []
        for dependency_y in stage_sub1_reducescatter_dependency_list:
            for dependency_x in dependency_y:
                for dependency_event in dependency_x:
                    if dependency_event not in cluster_reducescatter_events:
                        cluster_reducescatter_events.append(dependency_event)
        for dependency_y in stage_sub0_reducescatter_dependency_list:
            for dependency_x in dependency_y:
                for dependency_event in dependency_x:
                    if dependency_event not in cluster_reducescatter_events:
                        cluster_reducescatter_events.append(dependency_event)
        stage_reducescatter_dependency_list = [[cluster_reducescatter_events for _ in range(cluster_shapes[cluster_step][1])] for _ in range(cluster_shapes[cluster_step][0])]
        ...
        for dependency_y in stage_sub0_attention_weight_dependency_list:
            for dependency_x in dependency_y:
                for dependency_event in dependency_x:
                    if dependency_event not in cluster_attention_weight_events:
                        cluster_attention_weight_events.append(dependency_event)
        stage_attention_weight_dependency_list = [[cluster_attention_weight_events for _ in range(cluster_shapes[cluster_step][1])] for _ in range(cluster_shapes[cluster_step][0])]

        stage_attention_weight_reducescatter_dependency_list = []
        for sublist1, sublist2 in zip(stage_attention_weight_dependency_list, stage_reducescatter_dependency_list):
            merged_sublist = []
            for pair1, pair2 in zip(sublist1, sublist2):
                merged_sublist.append(pair1 + pair2)
            stage_attention_weight_reducescatter_dependency_list.append(merged_sublist)

        cluster_stage_attention_weight_reducescatter_dependency_list = []
        for sublist1, sublist2 in zip(cluster_attention_weight_reducescatter_dependency_list, stage_attention_weight_reducescatter_dependency_list):
            merged_sublist = []
            for pair1, pair2 in zip(sublist1, sublist2):
                merged_sublist.append(pair1 + pair2)
            cluster_stage_attention_weight_reducescatter_dependency_list.append(merged_sublist)
        record_event_tags.append(cluster_stage_attention_weight_reducescatter_dependency_list)                        

    if cluster_step < clusters_number - 1:

        cluster_sub1_pointtopoint_event_tag, cluster_sub1_pointtopoint_dependency_list = pointtopoint_api.cal_time(
            whole_nodes=node_network, current_event_tag=stage_sub0_reducescatter_event_tag, current_dependency_list=cluster_stage_attention_weight_reducescatter_dependency_list,
            source_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step],
            ...
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            message_flits=pass_bytes/cluster_sub1_shapes[cluster_step][0]/cluster_sub1_shapes[cluster_step][1],
            reduction=reduction_cores
        )

        cluster_sub0_pointtopoint_event_tag, cluster_sub0_pointtopoint_dependency_list = pointtopoint_api.cal_time(
            whole_nodes=node_network, current_event_tag=cluster_sub1_pointtopoint_event_tag, current_dependency_list=cluster_stage_attention_weight_reducescatter_dependency_list,
            source_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step],
            ...
            topology_x_limitation=node_k, topology_y_limitation=node_k,
            message_flits=pass_bytes/cluster_sub0_shapes[cluster_step][0]/cluster_sub0_shapes[cluster_step][1],
            reduction=reduction_cores
        )
        ...
        cluster_pointtopoint_event_tag = cluster_sub1_pointtopoint_event_tag
        ...
        stage_sub1_pointtopoint_event_tag, stage_sub1_pointtopoint_dependency_list = pointtopoint_api.cal_time(
            whole_nodes=node_network, current_event_tag=cluster_sub0_pointtopoint_event_tag, current_dependency_list=cluster_stage_attention_weight_reducescatter_dependency_list,
            source_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step],
            target_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step+1],
            source_x_number=cluster_sub1_shapes[cluster_step][1], source_y_number=cluster_sub1_shapes[cluster_step][0],
            target_x_number=cluster_sub1_shapes[cluster_step+1][1], target_y_number=cluster_sub1_shapes[cluster_step+1][0],
        ...
        stage_sub0_pointtopoint_event_tag, stage_sub0_pointtopoint_dependency_list = pointtopoint_api.cal_time(
            whole_nodes=node_network, current_event_tag=stage_sub1_pointtopoint_event_tag, current_dependency_list=cluster_stage_attention_weight_reducescatter_dependency_list,
            source_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step],
            target_nodes_coordinates_list=cluster_sub0_nodes_lists[cluster_step+1],
            source_x_number=cluster_sub0_shapes[cluster_step][1], source_y_number=cluster_sub0_shapes[cluster_step][0],
            target_x_number=cluster_sub0_shapes[cluster_step+1][1], target_y_number=cluster_sub0_shapes[cluster_step+1][0],
        ...
        for sub0_dependency_y in stage_sub0_pointtopoint_dependency_list:
            for sub0_dependency_x in dependency_y:
                for sub0_dependency_event in dependency_x:

                    for sub1_dependency_y_idx, sub1_dependency_y in enumerate(stage_sub1_pointtopoint_dependency_list):
                        for sub1_dependency_x_idx, sub1_dependency_x in enumerate(sub1_dependency_y):

                            if dependency_event not in stage_sub1_pointtopoint_dependency_list[sub1_dependency_y_idx][sub1_dependency_x_idx]:
                                stage_sub1_pointtopoint_dependency_list[sub1_dependency_y_idx][sub1_dependency_x_idx].append(sub0_dependency_event)

        for sub1_dependency_y in stage_sub1_pointtopoint_dependency_list:
            for sub1_dependency_x in dependency_y:
                for sub1_dependency_event in dependency_x:

                    for sub0_dependency_y_idx, sub0_dependency_y in enumerate(stage_sub0_pointtopoint_dependency_list):
                        for sub0_dependency_x_idx, sub0_dependency_x in enumerate(sub0_dependency_y):

                            if dependency_event not in stage_sub0_pointtopoint_dependency_list[sub0_dependency_y_idx][sub0_dependency_x_idx]:
                                stage_sub0_pointtopoint_dependency_list[sub0_dependency_y_idx][sub0_dependency_x_idx].append(sub1_dependency_event)

        stage_pointtopoint_event_tag = stage_sub1_pointtopoint_event_tag
        cluster_pointtopoint_events = []
        for dependency_y in stage_sub0_pointtopoint_dependency_list:
            for dependency_x in dependency_y:
                for dependency_event in dependency_x:
                    if dependency_event not in cluster_pointtopoint_events:
                        cluster_pointtopoint_events.append(dependency_event)
        for dependency_y in stage_sub1_pointtopoint_dependency_list:
            for dependency_x in dependency_y:
                for dependency_event in dependency_x:
                    if dependency_event not in cluster_pointtopoint_events:
                        cluster_pointtopoint_events.append(dependency_event)
        stage_pointtopoint_dependency_list = [[cluster_pointtopoint_events for _ in range(cluster_shapes[cluster_step+1][1])] for _ in range(cluster_shapes[cluster_step+1][0])]
...
'''
    ...
    file_path = os.path.join(target_folder_py, 'te_base_' + hardware_config + '_moe.py')
