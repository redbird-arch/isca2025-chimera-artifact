...
hardware_configs = ['a100', 'dojo1', 'dojo2', 'tpuv3']
...
'''
    ...
    code += '''
        if cluster_idx == 0:
            ...
            if layer_step == 0:
                cluster_allreduce_event_tag, cluster_allreduce_dependency_list = allreduce_api.cal_time(
                    whole_nodes=node_network, current_event_tag=stage_allgather_event_tag, current_dependency_list=cluster_stage_allgather_dependency_list,
                    source_nodes_coordinates_list=cluster_nodes_lists[cluster_step],
                    source_x_number=cluster_shapes[cluster_step][1], source_y_number=cluster_shapes[cluster_step][0],
                    topology_x_limitation=node_k, topology_y_limitation=node_n,
                    message_flits=pass_bytes,
        ...
        if cluster_idx == 0:
            if layer_step == 0:
                stage_allgather_event_tag, stage_allgather_dependency_list = allgather_api.cal_time(
            ...
            if layer_step == 0:
                stage_allgather_event_tag, stage_allgather_dependency_list = allgather_api.cal_time(
                    whole_nodes=node_network, current_event_tag=cluster_attention_weight_event_tag, current_dependency_list=cluster_stage_allgather_dependency_list,
                    source_nodes_coordinates_list=cluster_nodes_lists[cluster_step],
                    source_x_number=cluster_shapes[cluster_step][1], source_y_number=cluster_shapes[cluster_step][0],
                    topology_x_limitation=node_k, topology_y_limitation=node_n,
                    message_flits=pass_bytes,
        ...
        if layer_step == 0:
            stage_mlp_input_event_tag, stage_mlp_input_dependency_list = compute_2d_base(
                whole_nodes=node_network, current_event_tag=stage_allgather_event_tag, current_dependency_list=stage_allgather_dependency_list,
                source_nodes_coordinates_list=cluster_nodes_lists[cluster_step],
                source_x_number=cluster_shapes[cluster_step][1], source_y_number=cluster_shapes[cluster_step][0],
                topology_x_limitation=node_k, topology_y_limitation=node_n,
                message_flits=mlp_input_backward_bytes/cluster_shapes[cluster_step][0]/cluster_shapes[cluster_step][1],
            ...
            stage_mlp_input_event_tag, stage_mlp_input_dependency_list = compute_2d_base(
                whole_nodes=node_network, current_event_tag=stage_allgather_event_tag, current_dependency_list=stage_weight_allgather_dependency_list,
                source_nodes_coordinates_list=cluster_nodes_lists[cluster_step],
                source_x_number=cluster_shapes[cluster_step][1], source_y_number=cluster_shapes[cluster_step][0],
                topology_x_limitation=node_k, topology_y_limitation=node_n,
                message_flits=mlp_input_backward_bytes/cluster_shapes[cluster_step][0]/cluster_shapes[cluster_step][1],
        ...
        stage_weight_allgather_dependency_list = []
        for sublist1, sublist2 in zip(stage_mlp_weight_dependency_list, stage_allgather_dependency_list):
            merged_sublist = []
            for pair1, pair2 in zip(sublist1, sublist2):
                merged_sublist.append(pair1 + pair2)
            stage_weight_allgather_dependency_list.append(merged_sublist)
    ...
    if cluster_step < clusters_number - 1:
        ...
        stage_manytomany_event_tag, stage_manytomany_dependency_list = manytomany_api.cal_time(
            whole_nodes=node_network, current_event_tag=cluster_allgather_event_tag, current_dependency_list=cluster_stage_attention_dependency_list,
            source_nodes_coordinates_list=cluster_nodes_lists[cluster_step],
            target_nodes_coordinates_list=cluster_nodes_lists[cluster_step+1],
            source_x_number=cluster_shapes[cluster_step][1], source_y_number=cluster_shapes[cluster_step][0],
            target_x_number=cluster_shapes[cluster_step+1][1], target_y_number=cluster_shapes[cluster_step+1][0],
        ...
        stage_allgather_event_tag, stage_allgather_dependency_list = allgather_api.cal_time(
            whole_nodes=node_network, current_event_tag=stage_manytomany_event_tag, current_dependency_list=stage_manytomany_dependency_list,
            source_nodes_coordinates_list=cluster_nodes_lists[cluster_step+1],
            source_x_number=cluster_shapes[cluster_step+1][1], source_y_number=cluster_shapes[cluster_step+1][0],
            topology_x_limitation=node_k, topology_y_limitation=node_n,
            message_flits=pass_bytes,
        ...
        cluster_stage_allgather_dependency_list = []
        for sublist1, sublist2 in zip(cluster_allgather_dependency_list, stage_allgather_dependency_list):
            merged_sublist = []
            for pair1, pair2 in zip(sublist1, sublist2):
                merged_sublist.append(pair1 + pair2)
            cluster_stage_allgather_dependency_list.append(merged_sublist)
        record_event_tags.append(cluster_stage_allgather_dependency_list)
