...
medium_nodes = None
medium_shape = [0, 0]
target_nodes = [(0, 2), (0, 3), (1, 2), (1, 3)]
target_shape = [2, 2]
data_parallelism_degree = None
top_k = None

whole_flits = 16777216
...
node_network, noc_network = build(
    ...
    cfg_filepath=os.path.join(file_path, "../cfg/mesh_5_2_50_100.cfg")
)

ps = pipeline_sequence(
    topology=("mesh2d"), 
...
ps_fusion_event_tag, ps_fusion_dependency_list = ps.fusion(
...
with open(os.path.join(file_path,"../txt/"+os.path.splitext(os.path.basename(__file__))[0]+'.txt'), 'w') as f:
    # pickle.dump(ps_fusion_run_cost, f)
    f.write(str(ps_fusion_run_cost))
