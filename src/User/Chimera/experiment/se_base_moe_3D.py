...
'''
    ...
    code += '''
        ...
        cluster_allreduce_event_tag = cluster_sub1_allreduce_event_tag
        ...
        for dependency_z in cluster_se_dependency_list:
            for dependency_y in dependency_z:
                for dependency_x in dependency_y:
                    for dependency_event in dependency_x:
                        if dependency_event not in cluster_sub0_se_events:
                            cluster_sub0_se_events.append(dependency_event)
        cluster_sub0_se_dependency_list = [[[cluster_sub0_se_events for _ in range(cluster_sub0_shapes[cluster_step][2])] for _ in range(cluster_sub0_shapes[cluster_step][1])] for _ in range(cluster_sub0_shapes[cluster_step][0])]
        ...
        cluster_sub1_se_dependency_list = [[[cluster_sub1_se_events for _ in range(cluster_sub1_shapes[cluster_step][2])] for _ in range(cluster_sub1_shapes[cluster_step][1])] for _ in range(cluster_sub1_shapes[cluster_step][0])]
    ...
    if cluster_step < clusters_number - 1:
        ...
        cluster_sub1_pointtopoint_event_tag, cluster_sub1_pointtopoint_dependency_list = pointtopoint_api.cal_time(
            whole_nodes=node_network, current_event_tag=cluster_sub0_pointtopoint_event_tag, current_dependency_list=cluster_sub1_allgather_dependency_list,
            source_nodes_coordinates_list=cluster_sub1_nodes_lists[cluster_step],
